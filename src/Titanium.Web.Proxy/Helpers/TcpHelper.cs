...
namespace Titanium.Web.Proxy.Helpers
{
    ...
    internal class TcpHelper
    {
        ...
        internal static unsafe int GetProcessIdByLocalPort(AddressFamily addressFamily, int localPort)
        {
            ...
            if (NativeMethods.GetExtendedTcpTable(tcpTable, ref tcpTableLength, false, addressFamilyValue, allPid, 0) !=
                0)
                try
                {
                    tcpTable = Marshal.AllocHGlobal(tcpTableLength);
                    if (NativeMethods.GetExtendedTcpTable(tcpTable, ref tcpTableLength, true, addressFamilyValue,
                            allPid,
                            0) == 0)
                    {
