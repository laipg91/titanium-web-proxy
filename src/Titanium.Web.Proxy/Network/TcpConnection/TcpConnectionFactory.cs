    ...
    internal class TcpConnectionFactory : IDisposable
    {
            ...
            SslApplicationProtocol applicationProtocol)
        {
            ...
            if (upStreamProxy == null)
            {
                if (isSocks)
                {
                    upStreamProxy = server.UpStreamSocksProxy;
                ...
                {
                    upStreamProxy = server.UpStreamHttpProxy;
                }
            }
        ...
        /// <param name="cancellationToken">The cancellation token for this async task.</param>
        /// <returns></returns>
        internal async Task<TcpServerConnection?> GetServerConnection(ProxyServer proxyServer,
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (upStreamProxy == null)
            {
                if (isSocks)
                {
                    upStreamProxy = proxyServer.UpStreamSocksProxy;

                }
                else if (isHttps)
                {
                    upStreamProxy = proxyServer.UpStreamHttpsProxy;
                }
                else
                {
                    upStreamProxy=proxyServer.UpStreamHttpProxy;
                }
            }
        ...
        private async Task<TcpServerConnection?> CreateServerConnection(string remoteHostName, int remotePort,
            Version httpVersion, bool isHttps, SslProtocols sslProtocol,
            ...
            bool prefetch, CancellationToken cancellationToken)
        {
            ...
            try
            {
                    ...
                    if (sessionArgs != null && proxyServer.CustomUpStreamProxyFailureFunc != null)
                    {
                        ...
                        if (newUpstreamProxy != null)
                        {
                            ...
                            return await CreateServerConnection(remoteHostName, remotePort, httpVersion, isHttps,
                                sslProtocol, applicationProtocols, isConnect, proxyServer, sessionArgs,
                                upStreamEndPoint,
                ...
                stream = new HttpServerStream(proxyServer, new NetworkStream(tcpServerSocket, true),
                    ...
                    (isConnect || isHttps))
                {
                    ...
                    if (httpStatus.StatusCode != 200 && !httpStatus.Description.EqualsIgnoreCase("OK")
                                                     && !httpStatus.Description.EqualsIgnoreCase(
        ...
        private async Task ClearOutdatedConnections()
        {
            while (runCleanUpTask)
                ...
                {
                    Server.ExceptionFunc?.Invoke(new Exception("An error occurred when disposing server connections.",
                        e));
                }
                finally
                {
