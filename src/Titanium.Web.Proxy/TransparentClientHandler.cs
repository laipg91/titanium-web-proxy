                ...
                if (clientHelloInfo != null)
                {
                    ...
                    if (endPoint.DecryptSsl && args.DecryptSsl)
                    {
                        ...
                        catch (Exception e)
                        {
                            ...
                            var session = new SessionEventArgs(this, endPoint, clientStream, null,
                                cancellationTokenSource);
                            throw new ProxyConnectException(
                                $"Couldn't authenticate host '{httpsHostName}' with certificate '{certName}'.", e,
                                session);
                        }
                    }
                    else
                    {
                        ...
                        bool isSocks = sessionArgs.IsSocks;
                        IExternalProxy? externalProxy;
                        if (isSocks)
                        {
                            externalProxy = UpStreamSocksProxy;
                        }
                        else
                        {
                            externalProxy = UpStreamHttpsProxy;
                        }
                        var connection = (await TcpConnectionFactory.GetServerConnection(this,
                            args.ForwardHttpsHostName,
                            args.ForwardHttpsPort,
                            HttpHeader.VersionUnknown, false, null,
                            true, sessionArgs, UpStreamEndPoint,
                            externalProxy, true, false, cancellationToken))!;
